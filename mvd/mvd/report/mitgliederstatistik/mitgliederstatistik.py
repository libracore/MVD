...
def get_columns(filters):
    ...
    return[
        ...
        {"label": _("Region"), "fieldname": "region", "fieldtype": "Link", "options": "Region", "width": 0}
    ]

def get_data(filters):
    data = []
    typ_filter = get_typ_filter(filters)
    region_filter = get_region_filter(filters)
    data, stand_1, neumitglieder, zuzueger = get_stand(filters, data, typ_filter, region_filter)
    data, wegzueger_qty = get_wegzueger(filters, data, typ_filter, region_filter)
    data, kuendigungen_qty = get_kuendigungen(filters, data, typ_filter, region_filter)
    data, korrektur = get_korrektur_wegzug_kuendigung(filters, data, typ_filter, region_filter)
    data, ausschluesse_qty = get_ausschluesse(filters, data, typ_filter, region_filter)
    data, gestorbene_qty = get_gestorbene(filters, data, typ_filter, region_filter)
    data = get_stand_2(filters, data, stand_1, neumitglieder, zuzueger, wegzueger_qty, kuendigungen_qty, ausschluesse_qty, gestorbene_qty, korrektur, typ_filter, region_filter)
    data = get_spaetere_eintritte(filters, data, typ_filter, region_filter)
    data = get_vorgem_kuendigungen_2(filters, data, typ_filter, region_filter)
    data = get_ueberschrift_1(filters, data)
    data = get_erben(filters, data, typ_filter, region_filter)
    data = get_ohne_mitgliednummer(filters, data, typ_filter, region_filter)
    data = get_nicht_bezahlt(filters, data, typ_filter, region_filter)
    data = get_ueberschrift_2(filters, data,)
    data = get_interessiert(filters, data, typ_filter, region_filter)
    data = get_angemeldet(filters, data, typ_filter, region_filter)
    return data

def get_stand(filters, data, typ_filter, region_filter):
    '''
    ...
    neumitglieder = frappe.db.sql("""SELECT
                                    ...
                                    {typ_filter}
                                    {region_filter}""".format(from_date=filters.from_date, \
                                    to_date=filters.to_date, sektion_id=filters.sektion_id, typ_filter=typ_filter, region_filter=region_filter), as_dict=True)[0].qty
    ...
    data.append(
        {
            ...
            'region': get_region_value(filters)
        })
    data.append(
        {
...
def get_wegzueger(filters, data, typ_filter, region_filter):
    wegzueger = frappe.db.sql("""SELECT
                                ...
                                {typ_filter}
                                {region_filter}""".format(sektion_id=filters.sektion_id, \
                                from_date=filters.from_date, to_date=filters.to_date, typ_filter=typ_filter, region_filter=region_filter), as_dict=True)[0].qty
    data.append(
        {
            'mitglieder': 'Wegzüge',
...
def get_kuendigungen(filters, data, typ_filter, region_filter):
...
def get_korrektur_wegzug_kuendigung(filters, data, typ_filter, region_filter):
...
def get_ausschluesse(filters, data, typ_filter, region_filter):
...
def get_gestorbene(filters, data, typ_filter, region_filter):
...
def get_erben(filters, data, typ_filter, region_filter):
...
def get_stand_2(filters, data, stand_1, neumitglieder, zuzueger, wegzueger_qty, kuendigungen_qty, ausschluesse_qty, gestorbene_qty, korrektur, typ_filter, region_filter):
...
def get_spaetere_eintritte(filters, data, typ_filter, region_filter):
    spaetere_eintritte = frappe.db.sql("""SELECT
                                ...
                                {typ_filter}
                                {region_filter}""".format(sektion_id=filters.sektion_id, \
                                to_date=filters.to_date, typ_filter=typ_filter, region_filter=region_filter), as_dict=True)[0].qty
    data.append(
        {
...
def get_vorgem_kuendigungen_2(filters, data, typ_filter, region_filter):
    kuendigungen = frappe.db.sql("""SELECT
                                    COUNT(`name`) AS `qty`
                                FROM `tabMitgliedschaft`
                                WHERE `sektion_id` = '{sektion_id}'
...
def get_ohne_mitgliednummer(filters, data, typ_filter, region_filter):
    ohne_mitgliednummer = frappe.db.sql("""SELECT
                                ...
                                {typ_filter}
                                {region_filter}""".format(sektion_id=filters.sektion_id, typ_filter=typ_filter, region_filter=region_filter), as_dict=True)[0].qty
    data.append(
        {
...
def get_nicht_bezahlt(filters, data, typ_filter, region_filter):
    ...
    nicht_bezahlt_per_se = frappe.db.sql("""SELECT
                                            ...
                                            AND `mv_mitgliedschaft` IN (
                                                ...
                                                {typ_filter}
                                                {region_filter}
                                            )""".format(sektion_id=filters.sektion_id, year=year, to_date=filters.to_date, typ_filter=typ_filter, region_filter=region_filter), as_dict=True)[0].qty
...
def get_interessiert(filters, data, typ_filter, region_filter):
...
def get_angemeldet(filters, data, typ_filter, region_filter):
...
def get_region_value(filters):
    if hasattr(filters, 'region') and filters.region and filters.region not in ['Alle Regionen', 'Ohne Region']:
        return filters.region
    return ''

def get_region_filter(filters):
    region_filter = ''
    if hasattr(filters, 'region') and filters.region:
        if filters.region == 'Ohne Region':
            region_filter = "AND (`region` IS NULL OR `region` = '' OR `region` = 'None')"
        elif filters.region != 'Alle Regionen':
            region_filter = "AND `region` = '{0}'".format(filters.region)
