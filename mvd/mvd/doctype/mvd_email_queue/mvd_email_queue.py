...
def mvd_mail_flush():
    mail_queues = frappe.db.sql("""SELECT `name` FROM `tabMVD Email Queue` WHERE `status` NOT IN ('Send', 'Sending finished with errors') AND `blocked` != 1 ORDER BY `creation` ASC LIMIT 1""", as_dict=True)
