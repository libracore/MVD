...
frappe.ui.form.on('MVD Data Export', {
    refresh: frm => {
        frm.disable_save();
...
const export_data = frm => {
    let get_template_url = '/api/method/mvd.mvd.doctype.mvd_data_export.exporter.export_data';
    var export_params = () => {
        let columns = {};
    ...
    if (!frm.doc.bg_job){
        ...
        frappe.call({
            "method": "mvd.mvd.doctype.mvd_data_export.exporter.export_data",
            "args": export_params(),
            "callback": function(r) {
                frappe.msgprint(`Der Export wird als Background-Job ausgeführt<br>Sie finden das File im Anschluss <a href="/desk#List/File/Home">hier</a>`);
            }
        });
    }
};
