...
def export_data(doctype=None, parent_doctype=None, all_doctypes=True, with_data=False,
        select_columns=None, file_type='CSV', template=False, filters=None, limit=0, bg_job=0):
    if cint(bg_job) == 1:
        args = {
            'doctype': doctype,
        ...
        enqueue("mvd.mvd.doctype.mvd_data_export.exporter.export_data_as_bgj", queue='long', job_name='Data Export', timeout=5000, **args)
        return
    ...
    exporter = DataExporter(doctype=doctype, parent_doctype=parent_doctype, all_doctypes=all_doctypes, with_data=with_data,
        select_columns=select_columns, file_type=file_type, template=template, filters=filters, limit=limit, bg_job=bg_job)
    exporter.build_response()

def export_data_as_bgj(doctype=None, parent_doctype=None, all_doctypes=True, with_data=False,
        select_columns=None, file_type='CSV', template=False, filters=None, limit=0, bg_job=0):
    _doctype = doctype
    if isinstance(_doctype, list):
        _doctype = _doctype[0]
...
class DataExporter:
        ...
        if self.file_type == 'Excel':
            ...
            if cint(self.bg_job) != 1:
                ...
                _file = frappe.get_doc({
                    "doctype": "File",
                    "file_name": "EXPORT_{0}.csv".format(self.doctype),
                    "folder": "Home",
                    "is_private": 1,
                    "content": cstr(self.writer.getvalue())
                })
    ...
    def add_data(self):
        ...
        limit_page_length = cint(self.limit) if cint(self.limit) > 0 else None
        # get permitted data only
        self.data = frappe.get_list(self.doctype, fields=["*"], filters=self.filters, limit_page_length=limit_page_length, order_by=order_by)
    ...
    def build_response_as_excel(self):
        ...
        if cint(self.bg_job) != 1:
            ...
            _file = frappe.get_doc({
                "doctype": "File",
                "file_name": "EXPORT_{0}.xlsx".format(self.doctype),
                "folder": "Home",
                "is_private": 1,
