...
def get_termin_uebersicht(berater_in, von=None, bis=None):
    ...
    termine = frappe.db.sql("""
        SELECT
            ...
            `bt`.`bis`,
            `bt`.`art`,
            IFNULL(`bt`.`ort`, 'Keine Ort Angabe') AS `ort`,
            `bt`.`telefonnummer`,
            ...
            `mitgl`.`abweichende_objektadresse` AS `abweichende_objektadresse`,
            `mitgl`.`objekt_strasse` AS `objekt_strasse`,
            `mitgl`.`objekt_hausnummer`AS `objekt_hausnummer`,
            `mitgl`.`objekt_plz` AS `objekt_plz`,
            `mitgl`.`objekt_ort` AS `objekt_ort`,
            `bt`.`creation`,
            `bt`.`owner`,
            `bt`.`notiz`,
            '' AS `von_zeit`,
