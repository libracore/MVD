...
def get_csv_data(mw_export, query=False):
    ...
    if len(query_data) > 0:
        ...
        for entry in query_data:
            ...
            if int(entry.postfach) == 1:
                strasse_pf = " ".join(['Postfach', str(entry.postfach_nummer) or ''])
            else:
                strasse_pf = " ".join([entry.strasse or '', str(entry.nummer or '') + str(entry.nummer_zu or '')])
            plz_6 = entry.plz
            ort = entry.ort
            ...
            frappe.db.sql(
                ...
                SET `m_und_w_export` = '{0}'
                WHERE `name` = '{1}'
                """.format(mw_export, entry.name)
            )
            if commit_loop == 30:
                frappe.db.commit()
                commit_loop = 1
