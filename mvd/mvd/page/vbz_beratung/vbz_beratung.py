...
def get_open_data():
    open_data = {
        'beratung': {
            ...
            'r2': frappe.db.sql("""SELECT COUNT(`name`) AS `qty` FROM `tabBeratung` WHERE `status` = 'Open' AND `kontaktperson` = 'Rechtsberatung Pool (MVBE)' AND (`beratung_prio` != 'Hoch' OR `beratung_prio` IS NULL)""", as_dcit=True)[0].qty,
