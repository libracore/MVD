...
def get_open_data():
    open_data = {
        'beratung': {
            ...
            'r2': frappe.db.sql("""SELECT COUNT(`name`) AS `qty` FROM `tabBeratung` WHERE `status` = 'Open' AND `kontaktperson` = 'Rechtsberatung Pool (MVBE)' AND (`beratung_prio` != 'Hoch' OR `beratung_prio` IS NULL) AND `sektion_id` = 'MVBE'""", as_dict=True)[0].qty,
            ...
            'r9': len(frappe.get_list('Beratung', fields='name', filters={'status': ['not in', ['Rückfragen', 'Open', 'Zusammengeführt']], 'ungelesen': 1, 'kontaktperson': ['is', 'set']}, limit=100, distinct=True, ignore_ifnull=True)),
