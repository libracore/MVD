...
def get_open_data():
    ...
    open_data = {
        'beratung': {
            ...
            'r2_wohnen': len(frappe.get_list('Beratung', fields='name', filters={'status': ['in', ['Open', 'In Arbeit']], 'beratung_prio': ['not in', ['Hoch']], 'typ': 'Wohnen'}, limit=100, distinct=True)),
            'r2_business': len(frappe.get_list('Beratung', fields='name', filters={'status': ['in', ['Open', 'In Arbeit']], 'beratung_prio': ['not in', ['Hoch']], 'typ': 'Business'}, limit=100, distinct=True)),
            # Zeilen R3-R8 und P2-P4 sind im Template nicht mehr vorhanden -> Abfragen deaktiviert.
            # Beim Reaktivieren auch die Klick-Handler in der .js und die Zeilen in der .html wieder einkommentieren.
...
# Nur von den deaktivierten Zeilen P2-P4 benutzt -> mit diesen zusammen deaktiviert.
# def get_p2(user, typ):
