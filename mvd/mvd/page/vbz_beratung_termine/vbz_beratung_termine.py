...
def get_alle_beratungs_termine(user, free_only=0, beratungsort=None,
            ...
            if not erlaubte_sektionen or termin.sektion_id in erlaubte_sektionen:
                if not erb_block or termin.berater_in in kontaktperson_multi_user:
                    ...
                    termin_data = {
                        ...
                        'name_mitglied': """<a href="/desk#Form/Mitgliedschaft/{0}" target="_blank">{1} {2}<br>({3})</a>""".format(termin.mv_mitgliedschaft, frappe.db.get_value("Mitgliedschaft", termin.mv_mitgliedschaft, 'vorname_1'), frappe.db.get_value("Mitgliedschaft", termin.mv_mitgliedschaft, 'nachname_1'), frappe.db.get_value("Mitgliedschaft", termin.mv_mitgliedschaft, 'status_c')),
    ...
    if not cint(termine_gebucht) == 1:
        ...
        if cint(termine_heute) == 1:
            datum_filter = """
                AND `date` >= '{datum_von} 00:00:00'
                AND `date` <= '{datum_von} 23:59:59'
            """.format(datum_von=datum_von)
        else:
            datum_filter = """
                AND `date` >= '{datum_von}'
