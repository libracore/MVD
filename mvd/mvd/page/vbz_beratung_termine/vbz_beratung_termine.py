            ...
            if not erlaubte_sektionen or termin.sektion_id in erlaubte_sektionen:
                if not erb_block or termin.berater_in in kontaktperson_multi_user:
                    ...
                    termin_data = {
                        ...
                        'name_mitglied': """<a href="/desk#Form/Mitgliedschaft/{0}" target="_blank">{1}<br>({2})</a>""".format(termin.mv_mitgliedschaft, name_mitglied, frappe.db.get_value("Mitgliedschaft", termin.mv_mitgliedschaft, 'status_c')),
