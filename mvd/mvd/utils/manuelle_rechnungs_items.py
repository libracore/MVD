...
def get_item_price(item, customer_customer=None):
    ...
    if customer_group == 'Mitglied':
        ...
        if parent_rule:
            ...
            if len(request) > 0:
                price = request[0].rate
    else:
        request = frappe.db.sql("""SELECT `price_list_rate` FROM `tabItem Price` WHERE `price_list` = 'Standard Selling' AND `item_code` = '{item}' AND `valid_from` <= CURDATE() ORDER BY `valid_from` DESC LIMIT 1""".format(item=item), as_dict=True)
        if len(request) > 0:
            price = request[0].price_list_rate
