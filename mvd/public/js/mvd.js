...
// Funktion zum Formattieren von Telefonnummern
frappe.provide('frappe.mvd.apply_phone_format');
frappe.mvd.apply_phone_format = function(control) {
    if (!control || !control.$input) return;

    control.$input.off('input').on('input', function(e) {
        ...
        'callback': function(r) {
            ...
            frappe.call({
                ...
                callback: function(verfuegbarkeiten) {
                    ...
                    var tel = config.telefon || '';
                    var defaultArt = config.sektion_id == 'MVBE' ? 'telefonisch' : 'persönlich';
                            ...
                            frappe.call({
                                ...
                                callback: function(r) {
                                    ...
                                    if (r.message) {
                                        ...
                                        if (d.get_value('neue_beratung') != 1) {
                                            kwargs['beratung'] = d.get_value('beratung');
                                        }
...
frappe.mvd.render_nextcloud_files_tree = function(frm) {
    ...
    .then(r => {
        if (frm._nextcloud_request_id !== request_id || !r.message || r.message.nc_enabled != 1) {
            return;
        }

        frm.set_df_property('section_nextcloud', 'hidden', 0);
        ...
        $('<button type="button" class="btn btn-xs btn-default"><i class="fa fa-folder-open"></i> Alle Ordner öffnen</button>')
            .appendTo($toolbar)
            .on("click", function() {
                frappe.mvd.render_nextcloud_files_tree(frm).then(function() {
                    return frappe.mvd.expand_all_nextcloud_folders(frm);
                });
            });
...
frappe.mvd.expand_all_nextcloud_folders = function(frm) {
