...
$(document).on('click', '.zip-cell', function() {
    const rsvmitglied = $(this).data('rsvmitglied');
    download_zip(rsvmitglied);
});
...
function download_zip(rsvmitglied) {
    _download_zip(rsvmitglied);
}

function _download_zip(rsvmitglied) {
    const url =
        "/api/method/mvd.www.va.meine_mandate.download_zip?mandat=" +
        encodeURIComponent(rsvmitglied);
